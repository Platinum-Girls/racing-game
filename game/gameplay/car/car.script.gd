...
@export_custom(0, "suffix:deg") var max_turn_angle: float = 0.75
@export_custom(0, "suffix:deg/s²") var turn_acceleration: float = .05
...
@export var drift_camera_angle_modifier: float = 4
@export var drift_camera_decay_rate: float = 0.1
@export var tire_spin_blend_dampening: float = 0.05
...
func _physics_process(delta: float) -> void:
	...
	if input_provider.is_jump_pressed() && floor_cast.is_colliding():
		velocity.y += jump_speed  #TODO: Investigate why backwards jumping is much shorter.
	else:
...
func apply_friction(delta: float) -> void:
	...
	if (
		xz_vel.length() < FRICTION_STOP_THRESHOLD
		and is_zero_approx(input_provider.get_acceleration_axis())
	):
...
func calculate_steer_direction(
	delta: float, multiplier: float = 1, cur_max_turn_angle: float = max_turn_angle
) -> void:
	...
	var target_steer_direction := (
		-steer_input * acceleration_sign * deg_to_rad(cur_max_turn_angle) * multiplier #TODO: Handle partial axis from controller?
	)

	#prevent turn_speed from accumulating TODO: Revise
	if abs(target_steer_direction - current_steer_direction) > 0:
		wheel_turn_speed = move_toward(
			wheel_turn_speed,
			deg_to_rad(cur_max_turn_angle),
			deg_to_rad(turn_acceleration) * delta
		)
	else:
		wheel_turn_speed = move_toward(
			wheel_turn_speed, 0, deg_to_rad(turn_acceleration) * delta
		)

	current_steer_direction = move_toward(
		current_steer_direction, target_steer_direction, wheel_turn_speed
	)
...
func _on_grounded_processing(_delta: float) -> void:
	...
	var blend: float = MathUtils.clamp01(fwd_vel * tire_spin_blend_dampening)
	if fwd_vel <= 0:  # reversing
...
func _on_drifting_physics_processing(delta: float) -> void:
	...
	perform_steering()
	# outward drifting, adds a little extra push for outward drifting
...
func _on_drifting_processing(delta: float) -> void:
	...
	camera_container.rotation.y = lerp_angle(
		camera_container.rotation.y,
		drifting_direction * drift_camera_angle_modifier,
		MathUtils.lerp_delta_decay(drift_camera_decay_rate, delta)
	)
...
func _on_normal_state_processing(_delta: float) -> void:
	visual.set_target_roll_rot(
		current_steer_direction * -input_provider.get_acceleration_axis_sign()
	)  #TODO: Probably want to make this proportional to speed.
	visual.set_speed_based_yaw(steer_input * deg_to_rad(max_turn_angle), velocity.dot(-basis.z))
...
func _on_air_physics_processing(delta: float) -> void:
	...
	quaternion = quaternion.slerp(
		Quaternion(Vector3.UP, rotation.y),
		MathUtils.lerp_delta_decay(AIR_RIGHTING_DECAY_RATE, delta)
	)
