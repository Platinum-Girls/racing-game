...
func _process(delta: float) -> void:
	speed_factor = lerpf(
		speed_factor, target_speed_factor, MathUtils.lerp_delta_decay(speed_smoothing, delta)
	)
...
func trigger_hop() -> void:
	animation_tree.set(
		&"parameters/hop_trigger/request", AnimationNodeOneShot.ONE_SHOT_REQUEST_FIRE
	)
